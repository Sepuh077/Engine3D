"""
Engine3D CLI - Command line interface for creating and managing projects.

Usage:
    engine3d startproject <name>    Create a new project
    engine3d build                  Build executable
    engine3d run                    Run the project
    engine3d editor                 Launch the editor
    engine3d --version              Show version
    engine3d --help                 Show help

Example:
    engine3d startproject mygame
    cd mygame
    engine3d run
    engine3d build --onefile
"""
...
# This allows 'from engine3d.types import Color' to work
_cli_file = os.path.abspath(__file__)
_engine3d_pkg_dir = os.path.dirname(_cli_file)  # engine3d/engine3d/
_engine3d_dir = os.path.dirname(_engine3d_pkg_dir)  # engine3d/
_project_root = os.path.dirname(_engine3d_dir)  # project root
...
MAIN_PY_TEMPLATE = '''"""
{project_name} - Engine3D Game
Generated by Engine3D CLI
"""
import sys
from pathlib import Path

# Add engine3d to path (if needed)
...
[tool.engine3d.build.backends.pyinstaller]
onefile = false
console = false
windowed = true

[tool.engine3d.build.backends.nuitka]
onefile = false
standalone = true
...
engine3d build --backend nuitka

# Build debug version with console
engine3d build --debug
```
...
# Add engine3d to path if needed (when engine3d is in parent directory)
try:
    from engine3d.build import BuildSystem
except ImportError:
    # Try common locations
    engine3d_paths = [
        Path(__file__).parent.parent / "engine3d",  # sibling to project
        Path(__file__).parent.parent.parent / "engine3d",  # src/engine3d
        Path(__file__).parent.parent,  # project is inside engine3d
    ]
    for path in engine3d_paths:
        if path.exists() and (path / "engine3d" / "build.py").exists():
            sys.path.insert(0, str(path.parent if path.name == "engine3d" else path))
            break
    from engine3d.build import BuildSystem
...
def create_project(project_name: str, target_dir: Optional[Path] = None) -> Path:
    ...
    print(f"  engine3d run        # Run the game")
    print(f"  engine3d build      # Build executable")
...
def cmd_editor(args) -> None:
    ...
    try:
        from .engine3d import run_editor
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  engine3d startproject mygame       Create a new project called 'mygame'
  engine3d run                       Run the current project
  engine3d build                     Build executable
  engine3d build --onefile           Build single executable file
  engine3d build --backend nuitka    Build with Nuitka
  engine3d editor                    Launch the editor
  engine3d --version                 Show version information
        """,
    )
